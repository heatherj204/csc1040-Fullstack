...
//                         >
//                         <TableCell align="center"><Button><Link to={{pathname: `/module/${details.code}`}} style={{textDecoration: 'none'}}>{details.code}</Link></Button></TableCell>
...
export default function ModList({ code }) {
    ...
    function getname(name_link) {
        if (!name_link) return [];
        return name_link.map(link => {
            const name = link.split('/').slice(-2, -1)[0]; // Extract name
            return name + " ";
        });
    }
                            ...
                            {module
                                ...
                                .map((details) => (
                                    <TableRow key={details.code} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                        <TableCell align="center">
                                            <Button>
                                                <Link to={`/module/${details.code}`} style={{ textDecoration: 'none' }}>
                                                    {details.code}
                                                </Link>
                                            </Button>
                                        </TableCell>
                                        ...
                                        <TableCell align="center">
                                            {getname(details.delivered_to).map((n, index) => (
                                                <Button key={index}>
                                                    <Link to={`/cohort/${n.trim()}`} style={{ textDecoration: 'none' }}>
                                                        {n}
                                                    </Link>
                                                </Button>
                                            ))}
